import {LastFeeAMLEntity, DefiEdgePoolsAndStrategiesEntity} from "../generated/schema"
...
import { addressZero, defiedgeFactoryAddress, factoryAddress } from "../src/constants"

export function handleChangeProtocolPerformanceFee(event: ChangeProtocolPerformanceFeeEvent): void {
    ...
    const factoryContract = DefiEdgeFactoryABI.bind(Address.fromString(defiedgeFactoryAddress))
    const isStrategy = factoryContract.isValidStrategy(event.params.strategyOrPool);
    ...
    if(isStrategy){
        ...
        for(let i = 0; i < defiEdgePoolsAndStrategies.strategies.length; i++){
            if(defiEdgePoolsAndStrategies.strategies[i] == event.params.strategyOrPool){
                strategyAddress =  Address.fromBytes(defiEdgePoolsAndStrategies.strategies[i])
                break
            }
        }
        ...
        const lastFeeAMLEntity = LastFeeAMLEntity.load(underlying) as LastFeeAMLEntity;
        ...
        const factoryFee = factoryContract.getProtocolPerformanceFeeRate(strategyContract.pool(), strategyAddress)
